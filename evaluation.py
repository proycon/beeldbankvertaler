...
def main():
    ...
    parser.add_argument('--inp',type=str,help='Input file', action='store',required=True)
    ...
    matrexsrcfile, matrextgtfile, matrexoutfile = prepare(open(args.inp,'r',encoding='utf-8'), open(args.ref,'r',encoding='utf-8'), open(args.out,'r',encoding='utf-8'), args.mtevaldir, args.workdir)

    outprefix = '.'.join(args.out.split('.')[:-1])

    if args.mtevaldir:
        mtscore(args.mtevaldir, matrexsrcfile, matrextgtfile, matrexoutfile, outprefix, args.workdir)





def prepare(inp, ref, out, mtevaldir, workdir):
    inp_it = iter(inp)
    ref_it = iter(ref)
    out_it = iter(out)
    ...
    for t,f in (('src',matrexsrc),('ref',matrextgt),('tst',matrexout)):
        ...
        f.write("<DOC docid=\"beeldenbankvertalingen\" sysid=\"beeldenbankvertalingen\">\n")

    while True:
        try:
            inp_s = next(inp_it).strip()
            ref_s = next(ref_it).strip()
            out_s = next(out_it).strip()
        except StopIteration:
            break

        matrexsrc.write("<seg id=\"" + str(ref_s.id) + "\">" + inp_s + "</seg>\n")
...
def mtscore(mtevaldir, sourcexml, refxml, targetxml, outprefix, WORKDIR = '.'):
